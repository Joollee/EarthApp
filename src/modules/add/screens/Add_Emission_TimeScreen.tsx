...
> = ({navigation, route}) => {
  ...
  const onPress = () => {navigation.navigate("add_emission_end")}
  return (
    <View style={ROOT}>
      <Text text="Transport" preset="h1" />
      <Text text={title} preset="h3" style={TEXT_TITLE} />
      <Text text="Duration" preset="h1" style={TEXT_DURATION} />
      <View style={INPUT}>
        <TouchableOpacity onPress={handlerClickDown}>
          <Icon icon="IC_CIRCLE_BUTTON" style={ICON} />
        </TouchableOpacity>
        <View style={VALUE}>
          <Text text={`${hour} h`} preset="h2" style={TEXT_VALUE} />
        </View>
